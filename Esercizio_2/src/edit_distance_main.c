...
//Loads the file to correct in the memory
...
//Loads the dictionary file in the memory
...
// function that returns the struct TermList of a reference string 
// with the correction strings with minimum edit distance
static TermList *min_edit_distance(char *examed_word, Array *dict)
{
  ...
  {
    char *correction = (char *)array_get(dict, i);
    int dist = edit_distance_dyn(examed_word, correction);
    if (dist <= termlist_get_edit_distance(list))
       termlist_add(list, correction);
  }
  return list;
}
...
static void test_with_edit_distance(const char *filepath_1, const char *filepath_2)
{
  ...
  list = (TermList **)malloc(sizeof(TermList *) * array_size(words));
  printf("\n-------TEXT CORRECTION-------\n\n");

  for (unsigned long i = 0; i < array_size(words); i++)
  {
    char *word_to_examine = (char *)array_get(words, i);
    list[i] = min_edit_distance(word_to_examine, dict);
