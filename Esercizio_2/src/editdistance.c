...
static int rec_edit_distance_dyn(char* s1,char* s2, int l1, int l2, int** matrix){
  ...
  //se i caratteri sono diversi
    ...
    if (matrix[l1 - 1][l2] == -1) 
      matrix[l1 - 1][l2] = edit_distance_dyn_rec(s1, s2, l1 - 1, l2, matrix);
    if (matrix[l1][l2 - 1] == -1) 
      matrix[l1][l2 - 1] = edit_distance_dyn_rec(s1, s2, l1, l2 - 1, matrix);
...
static int **initialize_matrix(int rows, int columns) {
  ...
  for (int i = 0; i <= rows; i++)
    matrix[i] = (int *) malloc(sizeof(int) * (columns + 1));
  for (int i = 0; i <= rows; i++) {
