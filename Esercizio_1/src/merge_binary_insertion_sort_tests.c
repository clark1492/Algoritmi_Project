...
static void test_sorting_int_array(void)
{
  ...
  merge_binary_insertion_sort(int_array, nitems, sizeof(int), compare_int, 0);

  for (unsigned long i = 0; i < nitems; i++)
    TEST_ASSERT_EQUAL_INT(ord_int[i], int_array[i]);

  merge_binary_insertion_sort(int_array, nitems, sizeof(int), compare_int, nitems);
  
  for (unsigned long i = 0; i < nitems; i++)
    TEST_ASSERT_EQUAL_INT(ord_int[i], int_array[i]);
}

static void test_sorting_string_array(void)
{
  ...
  merge_binary_insertion_sort(string_array, nitems, sizeof(char *), compare_string, 0);

  for (unsigned long i = 0; i < 4; i++)
    TEST_ASSERT_EQUAL_STRING(ord_string[i], string_array[i]);

  merge_binary_insertion_sort(string_array, nitems, sizeof(char *), compare_string, nitems);

  for (unsigned long i = 0; i < 4; i++)
    TEST_ASSERT_EQUAL_STRING(ord_string[i], string_array[i]);
}

static void test_sorting_double_array(void)
{
  ...
  merge_binary_insertion_sort(double_array, nitems, sizeof(double), compare_double, 0);

  for (unsigned long i = 0; i < nitems; i++)
    TEST_ASSERT_EQUAL(ord_double[i], double_array[i]);

  merge_binary_insertion_sort(double_array, nitems, sizeof(double), compare_double, nitems);
...
int main(void)
{
  ...
  RUN_TEST(test_sorting_int_array);
  RUN_TEST(test_sorting_double_array);
  RUN_TEST(test_sorting_string_array);
