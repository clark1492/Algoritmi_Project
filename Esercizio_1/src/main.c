...
static void test_with_merge_binary_insertion_sort_k_zero(const char *filename, int (*compare)(void *, void *))
{
  clock_t start,end;
  double time;
  Array *sa = array_create();
  load_array(filename, sa);
  Array* ra = memcpy(ra, sa, sizeof(sa));
  Array* ta = memcpy(ta, sa, sizeof(sa));
  printf("Merge BinaryInsertion Sort k=0\n");
  start = clock();
  merge_binary_insertion_sort(array_get_internal(sa), array_size(sa), sizeof(struct record *), compare, 0);
  end = clock();
  printf("%.0d",(end-start/CLOCKS_PER_SEC);
  printf("Merge BinaryInsertion Sort k=50\n");
  merge_binary_insertion_sort(array_get_internal(ra), array_size(ra), sizeof(struct record *), compare, 50);
  printf("Merge BinaryInsertion Sort k=100\n");
  merge_binary_insertion_sort(array_get_internal(ta), array_size(ta), sizeof(struct record *), compare, 100);
  print_array(sa);
  free_data(sa);
  free_data(ra);
  free_data(ta);
