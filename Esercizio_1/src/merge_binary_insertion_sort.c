...
void merge_binary_insertion_sort(void *base, size_t nitems, size_t memSize, compare fun, unsigned long k_value)
{
  ...
  if (nitems < 1)
  {
    fprintf(stderr, "merge_binary_insertion_sort: nitems parameter cannot be less than one\n");
    exit(EXIT_FAILURE);
  }
  if (fun == NULL)
  {
  ...
  if (k_value > nitems) 
  {
    fprintf(stderr, "merge_binary_insertion_sort: k_value cannot be greater than number of items of the array or less than zero\n");
    exit(EXIT_FAILURE);
  }
  ...
  if (temp_array == NULL)
  {
    fprintf(stderr, "merge_binary_insertion_sort: unable to allocate memory for the temporary arry\n");
    exit(EXIT_FAILURE);
  }
