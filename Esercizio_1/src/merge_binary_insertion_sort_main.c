...
{
  printf("sorting_app records.csv < 1 | 2 | 3 > < K_VALUE >\n");
...
static void print_array(Array *array)
{
  ...
  printf("\nWriting the sorted array in file/sorted_records.csv...\n");
  FILE *file = fopen("../file/sorted_records.csv", "w");
  if (file == NULL)
  {
...
/*
 ** Must be invoked with the filename path, the number of the field to be sorted 
 ** and the number of the length of the sublist that are sorted using BinaryInsertion Sort.
*/
